...
2. `Pixel Room`：承载房间布置/场景编辑能力（家具开关、摆放、主题等）。
3. `ImmersiveLive2DShell`：承载高清背景、Live2D 与右侧互动列。
4. 场景映射：`area/slot -> immersive background`，支持默认回退。
5. 入口策略：先支持主 Agent 点击进入，后扩展到任一 Agent。
6. 边界约束：深度互动按钮列不承载“房间布置”入口，避免与 Pixel Room 职责重叠。
...
5. 本阶段不引入“房间布置”按钮，布置能力保留在 Pixel Room。
...
1. 扩展 stat（信任、压力、活力等）。
2. 扩展 Pixel Room 的房间布置能力（主题、家具、布局方案与资产管理）。
