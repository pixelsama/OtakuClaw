...
- Input-handler 暴露 `/metrics` Prometheus 端点，覆盖连接状态 (`bilibili_connection_status`)、事件吞吐 (`bilibili_events_total`)、重连计数 (`bilibili_reconnect_total`) 以及看门狗触发次数 (`bilibili_watchdog_trigger_total`)。
- Bilibili WebSocket 客户端内置看门狗（`BILI_WATCHDOG_SECONDS`），若超过阈值未收到心跳或弹幕会主动重连。
- 支持通过 `BILI_ALERT_WEBHOOK` + 阈值/冷却参数在连续失败或看门狗触发时发送告警，便于挂接 Slack / Webhook。

### 开放平台回调接入

- 新增 `services/input-handler-python/callbacks/bilibili.py` FastAPI 服务，校验 `X-Bili-*` 签名头并将 `superChatMessage`/`gift` 事件发布到 `live.chat` 频道。
- 通过 `BILI_CALLBACK_SECRET` / `BILI_CALLBACK_KEY` 配置密钥，对事件 ID 进行 Redis Set 去重（`BILI_CALLBACK_DEDUPE_PREFIX`）。
- `docker-compose.yml` 中提供 `bilibili-callback` 服务定义，可结合 ngrok/反向代理暴露公网 URL。
- 回调链路与 WebSocket 链路共享 `LiveEvent` 结构，可在 Dialog Engine 侧复用优先级与致谢逻辑。
